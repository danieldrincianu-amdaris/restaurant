...
- [x] **Task 1: Create KitchenOrderCard component** (AC: 1, 2, 5, 7)
  ...
  - [x] 1.3 Time elapsed display with timer icon (e.g., "⏱️ 12 min")
...
- [x] **Task 2: Create OrderItemsList component** (AC: 3, 4, 8)
  ...
  - [x] 2.3 Show special instructions below item with emphasized styling (🗒️ icon, bg-amber-50)
...
├── components/
│   └── kitchen/
│       ├── KitchenOrderCard.tsx    # Full order card component
│       └── OrderItemsList.tsx      # Items list with special instructions
└── lib/
    └── timeUtils.ts                # Elapsed time formatting + hook
...
```
┌───────────────┐
│ #127 Table 5  │  ← Order ID (last 6 chars) + Table number
│ ⏱️ 3 min      │  ← Time elapsed with icon
│───────────────│
│ 2x Margherita │  ← Items with quantities
│ 1x Caesar     │
│ 1x Espresso   │
│───────────────│
│ Server: John  │  ← Server name footer
└───────────────┘
```
...
```
│ 2x Margherita Pizza                    │
│    └─ 🗒️ "No olives, extra cheese"    │  ← Emphasized with icon + background
```
...
- **Implementation**: Special instructions shown with 🗒️ icon, amber background (bg-amber-50), amber text (text-amber-800)
- **Test Coverage**:
...
- **Implementation**: 🗒️ icon in card header when hasSpecialInstructions is true, plus "Special instructions" badge in OrderItemsList
- **Test Coverage**:
  - Given order with special instructions
  - When KitchenOrderCard renders
  - Then 🗒️ icon visible in header with title attribute
...
**Security**: ✓ PASS (no concerns for read-only display)
**Performance**: ✓ PASS (efficient timer updates, minimal re-renders)
**Reliability**: ✓ PASS (error handling for missing data, cleanup functions)
**Maintainability**: ✓ PASS (clear structure, good separation of concerns, comprehensive tests)
**Usability**: ✓ PASS (clear visual hierarchy, special instructions emphasis, time updates)
**Accessibility**: ✓ PASS (role, aria-label, keyboard navigation, proper focus management)
...
**Gate**: PASS → [docs/qa/gates/3.5-kitchen-order-cards.yml](../qa/gates/3.5-kitchen-order-cards.yml)
